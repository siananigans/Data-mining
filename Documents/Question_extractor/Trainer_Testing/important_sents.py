"""
To pick out the most important sentences from a text based of tf-idf and a
similarity score.
...
def get_important(sents, number, path='../models/idf_model'):
    # Sort sentences based on score
    sorted_sents, word_scores = sort_sents(sents, path)
...
def sort_sents(sents, path):
    ...
    idf_model_file = open(path, 'rb')
    idf_model = pickle.load(idf_model_file)
    # Get max idf
    max_idf = max(idf_model.values())

    # Make frequency distrobution for t-f score
    symbols = ".;:@&*<>,-_`~()[]{}"
    for sent in sents:
        # Split into words
        ...
        # See i in frequency distrobution, if not append
