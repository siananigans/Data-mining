...
def test(ctx):
    ...
    ctx.run(
        f"coverage run --source {PACKAGE} -m pytest {TEST_PACKAGE} --color=yes",
        env={"ERROR_CODES_PATH": "error-codes.json"},
    )
...
@task
def runbare(ctx):
    ctx.run(f"python -m {PACKAGE}.main -c config.dev.json -e error-codes.json")
